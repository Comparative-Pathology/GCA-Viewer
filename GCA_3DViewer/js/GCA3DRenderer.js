...
GCA3DRenderer = function(wind, cont, pick) {
  ...
  this.setPosition = function(pmk0, pmk1, pdt,
			      ...
			      emk0, emk1, edt) {
let kk =0;
if(pmk0=="GUT_ATLAS_REG:28" && pmk1=="GUT_ATLAS_REG:72")
kk=5				
  ...
  */
  ...
  this.getReferenceName = function() {
    let name = self.referenceNamePrefix + self.nameSep +
               self._config.reference_surface.id;
    return(name);
  }
...
try{
	...
	console.log("File doesn't exit:" + url)
}
    return(obj);
  }
...
if(self._config.paths[path] == undefined){
console.log(self._config.paths.length+'  , ' + path, +'   ', pathIdx)	
return;
}
    self._curPath = path;
