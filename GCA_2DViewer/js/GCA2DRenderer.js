...
home_dir = './2DViewer'
  this._icons = {
    pin: {
      url: `${home_dir}/icons/pin.svg`,
//      url: 'icons/pin.svg',
      prg: undefined},
    cursor: {
      url: `${home_dir}/icons/cursor.svg`,
//      url: 'icons/cursor.svg',
  ...
  this.init = function(cfg) {
	let canvasId = 'canvas_' + self._container.id
    let c = document.createElement('canvas');
    c.setAttribute('id', canvasId);
    self._container.appendChild(c);
    self._canvas = new fabric.Canvas(canvasId, {
										selection: false,
										fireRightClick: true,
...
//    self._win.addEventListener('resize', this._onResize);
		new ResizeObserver(this._onResize).observe(self._container);
  }
  ...
  this._onResize = function() {
    if(self._isDefined(self._container) && self._isDefined(self._canvas)) {
      if(self._container.clientHeight>0 && self._container.clientWidth>0) {
        self._canvas.setHeight(self._container.clientHeight);
        self._canvas.setWidth(self._container.clientWidth);
        self._setZoom();
      }
  ...
  this._setZoom = function() {
    if(this._isDefined(self._canvas) && this._isDefined(self._ref_image)) {
      ...
      let dx = (self._canvas.width - s * self._ref_image.width) / 2;	
      let dy = (self._canvas.height - s * self._ref_image.height) / 2;
      let shift = new fabric.Point(dx, dy);
      if(self.shift) { 
        self._canvas.relativePan(self.shift);  // move image to origin before applying zoom
      }
//      self._canvas.zoomToPoint(new fabric.Point(x, y), s);
      self._canvas.zoomToPoint(new fabric.Point(0, 0), s);
      self._canvas.relativePan(shift);    // move image to centre
      self.shift = new fabric.Point(-dx, -dy);   
      if(self._debug) {
